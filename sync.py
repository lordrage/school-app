...
digest, messages, shifts and personal blocks are left alone. Pearson-sourced
assignments (all of MAT 151) are preserved, since Canvas has none of them.
...
def main():
    ...
    # Canvas covers CHE 110 and BIO 110 only. MAT 151 lives entirely in Pearson
    # MyLab, which has no API — those rows are written into school-data.json by
    # the local morning check-in and must survive this Canvas-only refresh.
    # Without this, every robot run silently deletes all of Mckay's math work.
    kept_pearson = [a for a in data.get("assignments", [])
                    if a.get("source") == "Pearson"]
    fresh = fetch_assignments()
    for a in fresh:
        a.setdefault("source", "Canvas")
    merged = fresh + kept_pearson
    merged.sort(key=lambda a: a["due"])
    data["assignments"] = merged
